...
&& cd manticore && git checkout manticore-3.0.2 && mkdir build && cd build
...
FROM ubuntu:bionic
RUN apt-get update && apt-get install -y mysql-client curl libmysqlclient20 libodbc1 libpq5 libexpat1  && rm -rf /var/lib/apt/lists/*
